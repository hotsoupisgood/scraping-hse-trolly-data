...
def scrape_hse_trolleygar(date_str=None):
    ...
    # Otherwise return the largest table (likely the main data table)
    if len(dfs) == 1:
        main_df = dfs[0]
    else:
        # Return the table with the most rows
        main_df = max(dfs, key=lambda x: len(x))

    # Add metadata
    main_df['scrape_date'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    main_df['report_date'] = date_str
...
def scrape_date_range(start_date, end_date, filename='hse_trolleygar_data.csv'):
    """
    ...
    if all_dfs:
        ...
        save_to_csv(combined_df, filename, append=False)
        return combined_df
    return None
